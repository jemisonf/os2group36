...
/*
 * Allocate a slob block within a given slob_page sp using best fit
 */
static void *slob_page_alloc(struct page *sp, size_t size, int align)
{
	...
	int bf_delta = 0;
	slob_t  *bf_prev = NULL, *bf_next = NULL, 
			*bf_aligned = NULL, *bf_cur = NULL;
	slobidx_t bf_bestPage;
	...
	for (prev = NULL, cur = sp->freelist; ; prev = cur, cur = slob_next(cur)) {
		...
		// Checks available room and best fit
		if ((avail >= (units + delta)) /* room enough? */
				&& (bf_cur == NULL
				||  (bf_bestPage > (avail - (units + delta))))) {
			bf_bestPage = avail - (units + delta);
			bf_delta = delta;
			bf_prev = prev;
			bf_aligned = aligned;
...
asmlinkage unsigned long sys_slob_used(void)
{
	struct page *sp;
	struct list_head *slob_list;
	unsigned long flags;
